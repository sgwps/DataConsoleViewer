...
class Program{
    ...
    static void Main(){
        ...
        IScreenAction<FileRecord>[] actions = new IScreenAction<FileRecord>[] {
            new SortingAction<FileRecord>((x, y) => x.Date.CompareTo(y.Date), new MenuOption(ConsoleKey.D, "sort by date")),
            new SortingAction<FileRecord>((x, y) => x.Name.CompareTo(y.Name), new MenuOption(ConsoleKey.N, "sort by name")),
            new SelectingAction<FileRecord, string>((x, y) => x.Name.Split(" ").Contains(y), i => i, new MenuOption(ConsoleKey.F, "find by name"), "enter the key word"),
            new SelectingAction<FileRecord, DateOnly>((x, y) => x.Date.Month == y.Month & x.Date.Year == y.Year, i => new DateOnly(int.Parse(i[..4]), int.Parse(i[4..6]), 1), new MenuOption(ConsoleKey.O, "find by date"), "Enter month in format YYYYMM")
